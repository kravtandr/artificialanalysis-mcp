...
| Пакет | npm: `artificialanalysis-mcp`, Docker: `ghcr.io/<owner>/artificialanalysis-mcp` |
...
3. Multi-arch Docker-образ (amd64/arm64) → `ghcr.io/<owner>/artificialanalysis-mcp`
   с тегами `vX.Y.Z`, `X.Y`, `latest`.
4. GitHub Release с автогенерированными заметками.

Шаги 2→3→4 выполняются **последовательно** (npm — самый «неоткатываемый» шаг,
он идёт первым; упавший Docker-push можно перезапустить). Шаги 3 и 4
идемпотентны: повторный запуск workflow по тому же тегу не падает
